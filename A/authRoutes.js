...
// Import the Event Bus for decoupled background tasks
const eventBus = require('../events/eventBus');

// 🚨 UPGRADE: Import the direct email sender for automated ICT Admin resets
...
router.post('/forgot-password', rateLimit({ windowMs: 15 * 60 * 1000, max: 10 }), async (req, res) => {
  try {
    ...
    const user = await User.findOne({
      ...
      $or: [ { username: contactData }, { email: contactData } ]
    });
    ...
    // Instantly return generic success to the user so they can't farm usernames
...
router.post('/reset-password/:token', async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(400).json({ success: false, message: 'Your reset link is invalid or has expired. Please request a new one.' });
    }
    ...
    // Also clear any lingering manual requests if they exist
    ...
    await logAudit({
      user: user, role: 'ICT_ADMIN', action: 'PASSWORD_RESET_SUCCESS', category: 'AUTH', severity: 'HIGH',
      details: `ICT Admin successfully reset their password via secure email token.`, req
    });

    res.status(200).json({ success: true, message: 'Password has been successfully updated. You may now securely log in.' });
  } catch (error) {
...
// ICT Admin route to override notification emails
