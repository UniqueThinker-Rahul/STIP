...
// 🚨 UPGRADE: Added dedicated function for the Automated ICT Admin Password Recovery Flow
exports.sendPasswordResetEmail = async ({ toEmail, adminName, resetUrl }) => {
  const subject = '⚠️ STIP Portal - ICT Admin Password Reset';
  const content = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #E2DDD4; border-radius: 10px; overflow: hidden;">
      <div style="background-color: #0D2B55; padding: 20px; text-align: center;">
        <h2 style="color: #ffffff; margin: 0;">System Security Alert</h2>
      </div>
      <div style="padding: 30px; background-color: #ffffff;">
        <p style="color: #333; font-size: 16px;">Hello <strong>${adminName}</strong>,</p>
        <p style="color: #555; line-height: 1.6;">
          A password reset request was initiated for your ICT Administrator account on the STIP Portal. 
          This exclusive feature allows you to bypass manual resets.
        </p>
        <div style="text-align: center; margin: 30px 0;">
          <a href="${resetUrl}" style="background-color: #059669; color: #ffffff; padding: 14px 28px; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 14px; display: inline-block;">
            Reset My Password
          </a>
        </div>
        <p style="color: #991B1B; font-size: 12px; font-weight: bold;">
          If you did not request this, please ignore this email. This secure link will automatically expire in 15 minutes.
        </p>
      </div>
    </div>
  `;
  ...
  try {
    ...
    console.error('Error sending ICT Admin reset link email:', error);
  }
};