...
function NewAppraisalForm() {
  ...
  // 🚨 NEW: Filter Year State for the quarter selection
  ...
  useEffect(() => {
    const fetchFormContext = async () => {
      try {
        ...
        // 🚨 UPGRADED: Check for Future Quarters correctly to find default active
  ...
  // 🚨 UPGRADED: Enhanced locking logic checks for future/upcoming dates properly
  ...
  // 🚨 NEW: Dynamic Handlers for Filter Year & Sequential Locking
  ...
  const isCriterionComplete = (critId) => {
    ...
    return val !== null && comment.trim().length >= 5;
  };
            ...
            <>
                ...
                <div className="p-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      {/* 🚨 UPGRADED: Dual Filter Quarter/Year Selection Matrix */}
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mt-4">
                ...
                <div className="p-4 space-y-3">
                  {/* 🚨 UPGRADED: Sequential Map with visual and functional locking */}
                  {CRITERIA.map((crit, idx) => {
                    ...
                    return (
                      <div key={crit.id} className={`border rounded-xl overflow-hidden transition-all ${isRated ? 'border-[#0D2B55]/20 bg-white' : isExpanded && unlocked ? 'border-[#0D2B55] bg-[#FAF8F4]/30' : 'border-gray-200 bg-white'} ${!unlocked ? 'opacity-60' : ''}`}>
                        ...
                        {/* Sequentially forces collapse if unlocked state is lost */}
                        {isExpanded && unlocked && (
