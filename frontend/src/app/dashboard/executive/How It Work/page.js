...
export default function AddNewStaff() {
  ...
  const [executiveList, setExecutiveList] = useState([]); // 🚨 NEW: State for the Executive List
  ...
  // Universal Custom Modal State for Errors & Warnings
  ...
  // 🚨 UPGRADED: Added execId to the base form state
  const [formData, setFormData] = useState({
    fn: '', mn: '', ln: '', title: '', office: '', co: '', mgrId: '', execId: '', hire: '', empId: '', isManager: false, isExecutive: false 
  });

  const [openDropdown, setOpenDropdown] = useState(null);
  const [searchQueries, setSearchQueries] = useState({ mgrId: '', execId: '', title: '', office: '', co: '' }); 
  const dropdownRef = useRef(null);

  const fetchInitialData = async () => {
    try {
      // 🚨 UPGRADED: Fetch the active executives alongside managers
      const [mgrRes, execRes, configRes] = await Promise.all([
      ...
      setExecutiveList(execRes.data?.data || []); // Save to state
  ...
  const saveNewStaff = async () => {
    // 🚨 UPGRADED: Destructured execId
    const { fn, mn, ln, title, office, co, mgrId, execId, hire, empId, isManager, isExecutive } = formData;
    
    // 🚨 UPGRADED: Validation strictly requires both Line Manager AND Executive for standard staff. 
    // If user IS an Executive, both dropdowns become optional.
    if (!fn || !ln || !title || !office || !co || !hire || !empId || (!isExecutive && !execId) || (!isManager && !isExecutive && !mgrId)) {
      showDialog('warning', 'Missing Information', 'Please fill in all required fields. Line Manager and Executive Oversight are required unless the staff is explicitly designated as an Executive Member.');
      return;
    }
    ...
    try {
      ...
      // 🚨 UPGRADED: Passed executiveTo into the creation payload
      const payload = {
        ...
        companyCode: co,
        dateOfHire: hire, 
        role: isExecutive ? "EXECUTIVE" : (isManager ? "MANAGER" : "EMPLOYEE"), 
        reportingTo: isExecutive ? null : (mgrId || null),
        executiveTo: isExecutive ? null : (execId || null),
  ...
  const clearForm = () => {
    setFormData({ fn: '', mn: '', ln: '', title: '', office: '', co: '', mgrId: '', execId: '', hire: '', empId: '', isManager: false, isExecutive: false });
  };
  ...
  return (
    <div className="max-w-[1200px] mx-auto pb-[60px] font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-[16px]">
        ...
        <div className="lg:col-span-2 bg-white border border-[#E2DDD4] rounded-[14px] overflow-hidden self-start shadow-sm">
          ...
          <div className="p-[20px]">
            ...
            {/* 🚨 UPGRADED: Added the Executive member dropdown in the same row layout */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-[12px] mb-[14px]">
              <div className="flex flex-col gap-[6px]">
                <label className="text-[11px] font-[800] text-[#0D2B55] flex justify-between items-center">
                  <span>Line Manager {!(formData.isManager || formData.isExecutive) && <span className="text-[#DC2626]">*</span>}</span>
                  {(formData.isManager || formData.isExecutive) && <span className="text-[9px] text-[#6b7280] font-normal uppercase">Optional</span>}
                </label>
              ...
              <div className="flex flex-col gap-[6px]">
                <label className="text-[11px] font-[800] text-[#0D2B55] flex justify-between items-center">
                  <span>Exec Oversight {!formData.isExecutive && <span className="text-[#DC2626]">*</span>}</span>
                  {formData.isExecutive && <span className="text-[9px] text-[#6b7280] font-normal uppercase">Optional</span>}
                </label>
                {renderSearchableDropdown('execId', executiveList, 'Assign to Executive...', (m) => {
                  if (!m) return '';
                  const fName = m.personalDetails?.firstName || m.firstName || '';
                  const lName = m.personalDetails?.lastName || m.lastName || '';
            ...
            <div className="flex flex-col gap-[12px] mb-[20px]">
              <div className="p-[16px] bg-[#F0F9FF] border border-[#BFDBFE] rounded-[10px]">
                <div className="flex items-center gap-[12px]">
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input 
                      ...
                      onChange={(e) => setFormData(prev => ({ ...prev, isManager: e.target.checked, isExecutive: e.target.checked ? false : prev.isExecutive }))}
                    />
              ...
              {/* Executive Member Toggle */}
              <div className="p-[16px] bg-[#FFF4F2] border border-[#FECDD3] rounded-[10px]">
                <div className="flex items-center gap-[12px]">
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input 
                      type="checkbox" 
                      ...
                      onChange={(e) => setFormData(prev => ({ ...prev, isExecutive: e.target.checked, isManager: e.target.checked ? false : prev.isManager }))}
                    />
                    <div className="w-11 h-6 bg-white peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-[#6b7280] peer-checked:after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#9F1239] border-[1.5px] border-[#FECDD3]"></div>
                  </label>
                  <div>
                    <div className="text-[13px] font-[800] text-[#9F1239] flex items-center gap-[6px]">
                      <ShieldCheck className="w-[14px] h-[14px] text-[#E11D48]" /> Designate as Executive Member
                    </div>
                    <div className="text-[11px] text-[#E11D48] leading-snug mt-[2px]">
                      If enabled, this employee will be granted Executive access with oversight privileges. Executives do not require a Line Manager.
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* Universal Custom Modal for System Alerts & Errors */}
      {modalConfig.isOpen && (
