...
export default function QuarterlyScorecard() {
  const [curQ, setCurQ] = usePersistentFilter('qsc_selected_quarter', ''); 
  ...
  const [selectedYear, setSelectedYear] = usePersistentFilter('qsc_selected_year', new Date().getFullYear());
    ...
    const fetchDynamicQuarters = async () => {
      ...
      try {
        ...
        if (yearData && yearData.length > 0) {
          ...
          setCurQ((prevQ) => {
             // 🚨 UPGRADED: Type-safe check ensures localStorage strings are evaluated accurately
             const safePrevQ = prevQ ? String(prevQ) : '';
             if (!uniqueQuarters.some(m => m.label === safePrevQ)) {
  ...
  const downloadExcel = async () => {
    ...
    try {
      ...
      QS.forEach((qItem, i) => {
        ...
        sheet.cell(`J${rQs}`).value(has && cpVal > 0 ? cpVal : undefined);      
        sheet.cell(`K${rQs}`).value(maxCp * 0.48);         
        sheet.cell(`L${rQs}`).value(maxCp * 0.8);          
        sheet.cell(`M${rQs}`).value(maxCp);                
      });
