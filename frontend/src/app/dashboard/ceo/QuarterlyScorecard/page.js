    ...
    try {
      ...
      QKPAS.forEach((k, gi) => {
        ...
        k.inds.forEach((ind) => {
          ...
          const v = val(ind.c);
          const pct = ind.max && v ? (v / ind.max) * 100 : 0;
          body.push([
            ind.c, 
            ind.n, 
            ind.max, 
        ...
        body.push([
          ...
          a.toFixed(1),
          m ? `${((a / m) * 100).toFixed(0)}%` : "", 
          `Weight ${((m / TOTAL_MAX) * 100).toFixed(1)}%`,
          ""
        ]);
      ...
      autoTable(doc, {
        ...
        headStyles: { fillColor: [13, 43, 85], textColor: [255, 255, 255], fontStyle: "bold", fontSize: 7.6 },
        columnStyles: { 
          ...
          5: { cellWidth: 15 }, 
          6: { halign: "left", fontSize: 6.4, textColor: [100, 100, 100], cellWidth: 68 },
          7: { halign: "left", fontSize: 6.4, textColor: [100, 100, 100], cellWidth: 68 }
        },
        didParseCell: (h) => {
          ...
          if (subRows.indexOf(i) >= 0) {
            ...
            h.cell.styles.textColor = hexToRgb(KPA_COLORS[kpaIndex]);
    ...
    <div className="font-sans">
      <div className="flex flex-col md:flex-row justify-between items-start gap-3.5 mb-4 print:hidden">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          <button onClick={() => setDownloadMenu(true)} className="bg-[#C9A84C] hover:bg-[#e8c96a] text-[#0D2B55] px-3.5 py-2 rounded-[9px] text-[13px] font-bold cursor-pointer transition-colors shadow-sm">
            ⬇️ Download Report
          </button>
        </div>
      </div>
                        ...
                        <tr key={i.c} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="p-2.5 text-[13px] border-b border-gray-100 text-center align-middle">
                            <input 
                              ...
                              value={v !== null ? v : ''} 
                              onChange={(e) => handleActChange(i.c, e.target.value, i.max)}
                            />
                          </td>
                          ...
                          <td className="p-2.5 text-[13px] border-b border-gray-100 align-middle">
                            <div 
                              className={`text-[11.5px] text-[#92400E] min-h-[20px] px-2 py-1 rounded-md border border-[#FDE68A] bg-[#FEF9C3] transition-colors ${locked ? 'cursor-not-allowed opacity-70' : 'cursor-text hover:bg-[#FEF08A] hover:border-[#FACC15] focus:outline-none focus:border-[#C9A84C] focus:bg-white empty:before:content-[\'Add_important_text...\'] empty:before:text-[#D97706] print:p-0 print:border-none print:bg-transparent'}`}
                              contentEditable={!locked}
                              suppressContentEditableWarning
