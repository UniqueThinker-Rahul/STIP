...
import { AlertCircle, Download, FileText, Check } from 'lucide-react';
...
export default function ValidateAwards() {
  ...
  // In a real production app, CP_PCT and SCORECARD_LOCKED would be fetched from a CompanySettings API.
  const CP_PCT = 13.01;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter for only APPROVED appraisals using the secure workflow status
    ...
    let htmlContent = `
      <html>
        ...
        <body>
          ...
          <p><strong>Company Performance (CP):</strong> ${CP_PCT.toFixed(2)}%</p>
  ...
  const total = appraisals.reduce((s, a) => {
    ...
    const finalAw = (CP_PCT * iprf) * proRataValue;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-3">
          <button 
            ...
            disabled={!SCORECARD_LOCKED || CP_PCT === null || appraisals.length === 0}
          >
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-slate-900 rounded-xl p-5 shadow-sm border border-slate-800">
            <div>
              ...
              <div className="text-2xl font-bold text-amber-300">{CP_PCT.toFixed(2)}%</div>
            </div>
            <div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[1000px]">
                ...
                <tbody className="divide-y divide-slate-100">
                  {loading ? (
                    <tr>
                      <td colSpan="9" className="text-center py-12 text-slate-500 text-sm font-medium animate-pulse">
